...
> 24 小时延迟 / 丢包热力色块 · 微拟物玻璃质感 · 卡片 / 列表双视图 · monitor 状态面板主题
...
柔和双向阴影与高斯模糊背景，纯白（深色为墨蓝）卡片，24 小时延迟 / 丢包热力色块，冷暖色调贯穿全站，深浅色随系统自动切换。

## 特性

### 节点卡片

- 24 小时延迟 / 丢包分轨显示：所有探测线路按小时求均值后，延迟与丢包各一条独立轨道，每小时一格小圆角热力方块，每 6 格留一道空隙区分时段；五档低饱和色阶（延迟 <60ms 绿 / <110ms 草绿 / <180ms 黄 / <280ms 橙 / ≥280ms 红，丢包 <0.5% / <2% / <5% / <10% / ≥10%）
- 色块带顶部高光与底部浅投影，嵌在凹陷轨道里，悬停轻微抬起，延续整体的微拟物语言；没有数据的格子凹陷留白，每条轨道右侧直接标注平均值
- 逐格悬停可看该小时的延迟或丢包与探测数
- 没有探测数据的节点同样铺满 24 格空槽，卡片行高与有数据的节点完全一致；顶部一行汇总波动与探测数量
...
- 列表视图与卡片同构，每个节点一行：名称下面跟发行版图标与完整系统串（系统 · 虚拟化 · 架构，Debian / Ubuntu / CentOS / AlmaLinux / Rocky / Fedora / Arch / Alpine / openSUSE / Gentoo / OpenWrt / FreeBSD / Red Hat / macOS，Windows 退回落格图标）；CPU / 内存 / 硬盘三行堆叠，附核数、负载与真实用量，流量、下行 / 上行 / TCP·UDP、到期 / 续费各占一列；所有行共用同一套 `grid-template-columns`，列宽按权重分配（资源列最宽，延迟 / 网络 / 费用偏窄），每行的列起点、列终点因此天然对齐，延迟列不会被余量撑宽；延迟列直接复用卡片视图的 **24 小时双轨**（同一个 `HeatTrack` 组件、同一份多探测聚合数据，仅尺寸更紧凑），下方显示延迟区间与平均丢包
- 顶栏「进入后台 / 登录」与主题切换为两个相互独立的按钮，同样使用微拟物凹陷轨道样式，带按压缩回反馈
- 页面底部页脚卡片：左侧主题名与版本、站点名，右侧「源码」入口，下方分隔线内是推送方式与 monitor 署名，与节点卡同一套微拟物语言
- WebSocket 实时推送，断线自动轮询与重连；快照里没变化的节点沿用旧对象、指标抖动在展示精度内不触发重绘，滚动期间数据刷新先挂起、停滚后一次补上，状态圆点的呼吸光晕只走合成器动画，长列表滚动不易掉帧
...
- 信息卡片：硬件配置、运行状态、网络与流量、费用与到期，含系统、CPU、内存、硬盘、流量、费用、到期、Agent、主机名与 IP；到期颜色与主界面共用同一套阈值
