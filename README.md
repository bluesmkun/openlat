...
- 12 小时延迟 / 丢包分轨显示：所有探测线路按小时求均值后，延迟走冷色（蓝 → 紫）曲线，丢包走暖色（琥珀 → 红）曲线，两条独立轨道、色系分明，不会再混在一起；每条轨道右侧直接标注平均值
- 曲线为 Catmull-Rom 平滑，控制点收紧不会在尖峰外溢出；丢包轨道以 0 为基准，数据缺口如实断开，末格没有数据时以虚线标出「现在」
- 逐格悬停可看该小时的延迟或丢包与探测数
...
- 列表视图：名称与各指标列横向均匀铺开；延迟列直接复用卡片视图的 **12 小时双轨**（同一个 `TrackCurve` 组件、同一份多探测聚合数据，仅尺寸更紧凑），下方并排显示当前延迟与平均丢包，两个视图的呈现完全一致
