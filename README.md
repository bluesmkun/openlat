...
- 12 小时延迟 / 丢包波形：所有探测线路按小时求均值后合成一条曲线，延迟画成平滑的渐变面积波，丢包画成从基线长出的热力竖条，同一张图里同时看清趋势与量级
- 曲线为 Catmull-Rom 平滑，控制点收紧不会在尖峰外溢出；数据缺口如实断开，末格没有数据时以虚线标出「现在」
- 波形色调按平均延迟分档（绿 / 草绿 / 黄 / 橙 / 红），丢包竖条自下而上由暖黄渐至红，逐格悬停可看该小时的延迟、丢包与探测数
- 没有探测数据的节点同样保留完整轨道与时间轴，卡片行高与有数据的节点完全一致；波形下方一行汇总平均延迟、波动与丢包率
...
- 列表视图：名称与各指标列横向均匀铺开；延迟列直接复用卡片视图的 **12 小时波形**（同一个 `LatencyWave` 组件、同一份多探测聚合数据，仅尺寸更紧凑），并排显示当前延迟与平均丢包，两个视图的呈现完全一致
