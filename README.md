...
- 列表视图：系统版本只留一枚对应发行版的图标（Debian / Ubuntu / CentOS / AlmaLinux / Rocky / Fedora / Arch / Alpine / openSUSE / Gentoo / OpenWrt / FreeBSD / Red Hat / macOS，Windows 退回落格图标；完整版本名放进悬停提示），名称、流量等列固定宽度，剩余空间统一交给延迟列，右侧不再拥挤；延迟列直接复用卡片视图的 **24 小时双轨**（同一个 `HeatTrack` 组件、同一份多探测聚合数据，仅尺寸更紧凑），下方并排显示当前延迟与平均丢包
- 顶栏「进入后台 / 登录」与主题切换为两个相互独立的按钮，同样使用微拟物凹陷轨道样式，带按压缩回反馈
- 页面底部页脚卡片：左侧主题标识与版本、站点名，中间节点总数与在线 / 离线统计，右侧「源码 / 进入后台」入口，下方分隔线内是推送方式与 monitor 署名，与节点卡同一套微拟物语言
