...
- 列表视图与卡片同构，每个节点一行：名称下面跟发行版图标与完整系统串（系统 · 虚拟化 · 架构，Debian / Ubuntu / CentOS / AlmaLinux / Rocky / Fedora / Arch / Alpine / openSUSE / Gentoo / OpenWrt / FreeBSD / Red Hat / macOS，Windows 退回落格图标）；CPU / 内存 / 硬盘三行堆叠，附核数、负载与真实用量，流量、下行 / 上行 / TCP·UDP、到期 / 续费各占固定列，剩余空间统一交给延迟列；延迟列直接复用卡片视图的 **24 小时双轨**（同一个 `HeatTrack` 组件、同一份多探测聚合数据，仅尺寸更紧凑），下方显示延迟区间与平均丢包
