...
function Panel({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="rounded-2xl border neu p-4">
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? "neu-sm font-medium text-primary" : "text-muted-foreground hover:text-foreground",
      )}
    >
      {children}
    </button>
...
export function NodeDetail({ node, latency, onBack }: { node: Node; latency?: Latency; onBack: () => void }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-2">
        <button
          onClick={onBack}
          className="neu-track neu-press inline-flex h-8 shrink-0 items-center gap-1 rounded-lg border px-3 text-sm text-muted-foreground transition-colors hover:text-foreground"
        >
      ...
      <div className="space-y-3 border-t pt-4">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
          <div className="neu-inset flex items-center gap-0.5 rounded-lg p-0.5">
            {TABS.map((t) => (
          ...
          {tab === "latency" && pingSeries.length > 0 && (
            <label className="flex items-center gap-1.5 text-xs text-muted-foreground">
              ...
              <Select
                ...
                className="neu-sm border-transparent bg-transparent"
              >
        ...
        {!data ? (
          ...
          pingSeries.length === 0 ? (
            ...
            <div className="space-y-3">
                      ...
                      <ComposedChart data={pingRows}>
                        ...
                        <Tooltip
                          ...
                          contentStyle={{
                            ...
                            boxShadow: "0 18px 40px -24px rgb(0 0 0 / 0.45)",
                          }}
                          itemStyle={{ padding: 0 }}
                        />
              ...
              <div className="overflow-hidden rounded-2xl border neu">
      ...
      <div className="space-y-3 border-t pt-4">
        <div className="grid gap-3 md:grid-cols-2">
          {groups
            ...
            .map(({ label, icon: Icon, facts }) => (
              <section key={label} className="rounded-2xl border neu p-4">
                <h4 className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
                  <span className="grid size-6 place-items-center rounded-lg bg-primary/12 text-primary">
        ...
        {node.remark && (
          <p className="rounded-xl border bg-muted/40 px-3 py-2 text-sm whitespace-pre-wrap">{node.remark}</p>
        )}
      </div>
    </div>
  )
