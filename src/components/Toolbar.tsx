...
export function Toolbar({
  ...
  return (
    <div className="neu glass flex flex-wrap items-center gap-2 rounded-2xl border p-2">
      <div className="relative w-full sm:w-60">
        ...
        {query && (
          <button
            onClick={() => onQuery("")}
            title="清空"
            className="absolute top-1/2 right-1.5 grid size-4 -translate-y-1/2 place-items-center rounded-md text-muted-foreground transition hover:bg-muted hover:text-foreground"
          >
            <X className="size-3" />
          </button>
        )}
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  view === mode ? "neu-sm text-primary" : "text-muted-foreground hover:text-foreground",
                )}
              >
