...
function NodeMapBase({
  ...
  return (
    <div className={cn("rise relative flex flex-col overflow-hidden rounded-2xl border neu p-3.5", className)}>
      <span className="pointer-events-none absolute -top-10 -right-8 size-28 rounded-full bg-info/12 blur-3xl" />
          ...
          >
            <div className="flex items-center justify-between gap-2 px-1.5 pb-1 text-[11px] text-muted-foreground">
              ...
              <button
                ...
                title="关闭"
                className="grid size-4 shrink-0 place-items-center rounded-md transition hover:bg-muted hover:text-foreground"
              >
                <X className="size-3" />
              </button>
            </div>
            <div className="max-h-28 space-y-0.5 overflow-y-auto">
              {active.nodes.map((node) => (
                <button
                  ...
                  className="flex w-full items-center gap-2 rounded-lg px-2 py-1 text-left text-xs transition hover:bg-muted/70"
                >
                  <StatusDot node={node} />
                  ...
                  <span
                    className={cn(
                      "shrink-0 text-[10px]",
                      node.online ? "text-ok" : "text-destructive",
                    )}
                  >
                    {node.online ? "在线" : "离线"}
                  </span>
